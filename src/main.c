...
/*   Updated: 2025/12/02 17:12:26 by leramos-         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	init_data(t_pipex *data, char **av, char **envp)
{
	...
	if (pipe(data->pipefd) == -1)
		cleanup_and_exit(ERR_PIPE_FAIL, "Pipe failed");
	data->pstatus[0] = 0;
	data->pstatus[1] = 0;
}
...
int	main(int ac, char **av, char **envp)
{
	...
	if (ac != 5)
		cleanup_and_exit(ERR_INVALID_ARGS, "Use: infile c1 c2 outfile");
	...
	cleanup_and_exit(exit_status, NULL);
	return (0);
}
